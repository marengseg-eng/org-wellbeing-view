...
const ResultadoOrganizacional = () => {
  ...
  // Auto-generate 5W2H actions for factors above 50% + keep manually added ones
  const factorKeys = new Set(Object.values(FACTOR_5W2H_TEMPLATES) ? factorsWithAlert.map(f => f.key) : []);
  const acoes5w2h = useMemo<Acao5W2H[]>(() => {
    const autoGenerated = factorsWithAlert.map((f) => {
    ...
    const allFactorKeys = new Set(factorsWithAlert.map(f => f.key));
