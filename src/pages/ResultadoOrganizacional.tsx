...
const ResultadoOrganizacional = () => {
  ...
  // Auto-generate 5W2H actions from recommendations list + keep manually added ones
  const acoes5w2h = useMemo<Acao5W2H[]>(() => {
    const filteredRecs = recomendacoes.filter((r) => r.trim() && !r.startsWith("["));
    const autoGenerated: Acao5W2H[] = filteredRecs.map((rec, i) => {
      const recId = `rec-${i}`;
      const manual = acoes5w2hManual.find((a) => a.id === recId);
      if (manual) return manual;
      return {
        id: recId,
        what: rec,
        why: "Conforme recomendação técnica da avaliação psicossocial — NR-17 / NR-01",
        where: empresa ? `${empresa}${setorCustom ? ` — ${setorCustom}` : ""}` : "A definir",
        when: "30 dias — curto prazo",
        who: "SESMT, Gestores e RH",
        how: "Implementar conforme plano de ação e cronograma definido",
        howMuch: "A definir conforme diagnóstico",
        status: "pendente" as const,
      };
    });
    // Append manually added actions (those whose id doesn't start with rec-)
    const autoIds = new Set(autoGenerated.map((a) => a.id));
    const extraManual = acoes5w2hManual.filter((a) => !autoIds.has(a.id) && !a.id.startsWith("rec-"));
    return [...autoGenerated, ...extraManual];
  }, [recomendacoes, empresa, setorCustom, acoes5w2hManual]);
